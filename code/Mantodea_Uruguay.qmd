...
#     tm_borders(col='grey80')+
  tm_shape(mapa_observaciones, 
    ...
    tm_dots(fill='red', size=0.75,#'iconic_taxa',
...
new_users <- user_list_checked %>% 
    ...
    mutate(percent = (scales::label_percent(accuracy=0.1))(round(n/sum(n),3))) 
```
...
project_identifiers <- getProjectIdentifiers(id_proyecto)
...
threteaned_taxa <- datos_proyecto %>% 
  distinct(taxon_name, .keep_all = T) %>% 
  filter(taxon_rank=='species') %>% 
    group_by(threatened) %>%
    count() %>%
