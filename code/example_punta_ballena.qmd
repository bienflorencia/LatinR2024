...
id_proyecto <- 'biodiversidad-de-punta-ballena'
...
ggplotly(datos_proyecto %>% 
            ...
            add_count(iconic_taxa, day=day(time_observed_at),
                      name='records_per_day') %>%
           ...
           distinct(time_observed_at, .keep_all = T) %>% 
            mutate(text = paste0(
              ...
              '\nRegistros por hora: ', records_per_day)) %>% 
  ggplot(., aes(x=time_observed_at, y=records_per_day, text=text)) +
  geom_segment(aes(x=time_observed_at, xend=time_observed_at,
                   y=0, yend=records_per_day), color="grey35") +
  geom_point(col="#67a9cf", size=3) +
  scale_x_datetime(breaks = '1 day', date_labels="%d\n%H:%M") +
  theme_light()+
